...
class DataHandler {
    ...
    func updateData(modelType: ModelType, ifCategoryThenTagType: TagType?, ifTuneThenQueryString: String?) {
        ...
        if let urlString = urlString {
            guard let url = URL(string: urlString) else { return assertionFailure("string not converted to url") }
            ...
            let dataTask = session.dataTask(with: request) {(
                ...
                if let error = error {
                    ...
                    do {
                        guard let jsonArray = try JSONSerialization.jsonObject(with: jsonData, options: .mutableLeaves) as? [AnyObject] else {return}
